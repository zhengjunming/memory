				...
				for (FileItem fileItem : list) {
					...
					if (fileItem.isFormField()) {
						...
						if (filename == "") {
							...
							try {
								storeDaoImpl.addGoods(goods, store);
							} catch (Exception e) {
								e.printStackTrace();
							}
							response.setHeader("refresh",
									"2;URL=/zhengjunming_user_commodity_trading_system/ListAllStoreGoodsServlet");
							response.getWriter()
									.println(
											"<center><h1>上架商品成功，2秒后跳转到主页</h1></center>");
							return;
						}
						...
						try {
							if (service.imageFormatIsRight(filename) == StoreService.IMAGE_FORMAT_IS_ERROR) {
								...
								request.getRequestDispatcher(
										"/store/addGoods.jsp").forward(request,
										response);
								return;
							}
						} catch (Exception e) {
