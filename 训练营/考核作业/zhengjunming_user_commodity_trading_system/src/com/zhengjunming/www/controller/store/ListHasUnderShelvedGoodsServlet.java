 ...
 *
 */
@WebServlet("/ListHasUnderShelvedGoodsServlet")
public class ListHasUnderShelvedGoodsServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request,
		...
		try {
			count = daoImpl
					.getGoodsCountByStatusAndStoreId(
							GoodsStatus.HAS_BEEN_UNDER_SHELVED.getName(),
							store.getId());
		} catch (Exception e1) {
			e1.printStackTrace();
		}
		...
		try {
			goodses = daoImpl
					.ListGoodsByStatusAndStoreId(pageSize,
							Integer.parseInt(page),
							GoodsStatus.HAS_BEEN_UNDER_SHELVED.getName(),
							store.getId());
			request.setAttribute("goodses", goodses);
			request.getRequestDispatcher("/store/hasUnderShelvedGoods.jsp")
