 ...
 *
 */
@WebServlet("/ListHasOutGoodsServlet")
public class ListHasOutGoodsServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request,
		...
		try {
			count = daoImpl.getGoodsCountByStatusAndStoreId(
					GoodsStatus.OUT_OF_STOCK.getName(), store.getId());
		} catch (Exception e1) {
			e1.printStackTrace();
		}
		...
		try {
			goodses = daoImpl.ListGoodsByStatusAndStoreId(pageSize,
					Integer.parseInt(page), GoodsStatus.OUT_OF_STOCK.getName(),
					store.getId());
			request.setAttribute("goodses", goodses);
			request.getRequestDispatcher("/store/hasOutGoods.jsp").forward(
					request, response);
		} catch (Exception e) {
			e.printStackTrace();
		}
