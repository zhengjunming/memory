 ...
 *
 */
@WebServlet("/ListNotUnderShelvedGoodsServlet")
public class ListNotUnderShelvedGoodsServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request,
		...
		int count = 0;
		try {
			count = daoImpl.GetNotUnderShelvedGoodsCount(store);
		} catch (Exception e1) {
			e1.printStackTrace();
		}
		...
		try {
			goodses = daoImpl.ListNotUnderShelvedGoods(pageSize, Integer.parseInt(page), store);
			request.setAttribute("goodses", goodses);
			request.getRequestDispatcher("/store/underShelvedGoods.jsp").forward(
					request, response);
		} catch (Exception e) {
			e.printStackTrace();
		}
